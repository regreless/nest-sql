...
import { StateGraph, START, END, Annotation, MemorySaver, Send, Command, interrupt } from '@langchain/langgraph'
...
export class TechResearchService implements OnModuleInit {
    ...
    private buildGraph() {
        ...
        const parseTask = async (state: typeof TechResearchState.State) => {
            const res = await this.llm.invoke([
                new HumanMessage(`把以下技术选型问题拆成 3-4 个独立调研维度（性能、生态、开发体验、运维成本等）。
                    问题：${state.question}
                    输出 JSON（不要其他内容）：
        ...
        const researchAgent = async (state: typeof SingleResearchState.State) => {
            ...
            const res = await this.llm.invoke([
                new HumanMessage(`针对以下技术选型维度提供分析：
                问题：${state.question}
                维度：${state.dimension}
    ...
    async approve(threadId: string) {
        ...
        await this.graph.invoke(new Cmd({ resume: 'approved' }), { configurable: { thread_id: threadId } })
    ...
    async reject(threadId: string) {
        ...
        const { Command: Cmd } = await import('@langchain/langgraph')
        await this.graph.invoke(new Cmd({ resume: 'rejected' }), { configurable: { thread_id: threadId } })
